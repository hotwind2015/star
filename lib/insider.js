...
numbro.setDefaults('zh-CN', conf.numbro);
...
function displayMiscSummary(summary) {
    ...
    const limitedSummary = limit > 0 ? _.take(summary, limit) : summary;
...
function getTradings(json) {
    ...
    _.each(json , t=> {
        data.push({
            ...
            CHANGE_DATE: t.jyrq,
            CHANGE_NUM: +t.bdgs,
            CURRENT_AVG_PRICE: +t.bdjj,
            CHANGE_REASON: t.bdyy,
            _RATIO: +t.cgbdbl,
            HOLDSTOCK_NUM: +t.cgzs,
            _INSIDER: t.gdxm,
            DUTY: t.zw,
            _RELATION: t.gxlb,
        });
...
function queryInsider(code, cb) {
    ...
    request(option, (e, r, body) => {
        ...
        if (market === 'sz') {
            if (JSON.parse(body).length ===0) { return true}
            tradings = getTradings(JSON.parse(body)[0].data);
        } else {
